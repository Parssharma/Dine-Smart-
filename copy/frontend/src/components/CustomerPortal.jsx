...
export default function CustomerPortal({ initialTab = 'reserve' }) {
  const { user, isAuthenticated, isCustomer, login, register, getAuthHeaders } = useAuth();
  ...
  const getMinBookingDate = () => {
    ...
    const offset = minTime.getTimezoneOffset();
    const localMin = new Date(minTime.getTime() - (offset * 60 * 1000));
    return localMin.toISOString().split('T')[0];
  };

  const getMaxBookingDate = () => {
    const maxTime = new Date(Date.now() + 24 * 60 * 60 * 1000);
    const offset = maxTime.getTimezoneOffset();
    const localMax = new Date(maxTime.getTime() - (offset * 60 * 1000));
    return localMax.toISOString().split('T')[0];
  };

  const minDateStr = getMinBookingDate();
  const maxDateStr = getMaxBookingDate();

  // Read URL query params if user navigated from Quick Booking bar or Experience cards
  ...
  const computeEndTime = (startStr) => {
    if (!startStr) return '20:30';
    const [h, m] = startStr.split(':').map(Number);
    const totalMinutes = (h * 60 + (m || 0) + 90) % 1440;
  ...
  const [activeCustomerTab, setActiveCustomerTab] = useState(initialTab);
  ...
  // Booking Form State - dynamically initialized from authenticated customer
  const [customerName, setCustomerName] = useState(isCustomerAuth ? (user?.name || user?.email || '') : '');
  const [contact, setContact] = useState('');
  const [partySize, setPartySize] = useState(paramPartySize || '2');
  const [preference, setPreference] = useState(paramPref || '');
  const [bookingDate, setBookingDate] = useState(paramDate || todayStr);
  const [startTime, setStartTime] = useState(paramTime || '19:00');
  const [endTime, setEndTime] = useState(computeEndTime(paramTime || '19:00'));
  ...
  // Update customerName when customer logs in
  useEffect(() => {
  ...
  }, [user, isAuthenticated, isCustomer]);
  ...
  useEffect(() => {
    if (isCustomerAuth && activeCustomerTab === 'my-bookings') {
      fetchMyBookings();
    }
  }, [isCustomerAuth, activeCustomerTab]);
  ...
  // Update search form if searchParams change after mount
  useEffect(() => {
    const pSize = searchParams.get('partySize');
    const pDate = searchParams.get('date');
    const pTime = searchParams.get('time');
    const pPref = searchParams.get('preference');

    if (pSize || pDate || pTime || pPref) {
      if (pSize) setPartySize(pSize);
      if (pDate) setBookingDate(pDate);
      if (pTime) {
        setStartTime(pTime);
        setEndTime(computeEndTime(pTime));
      }
      if (pPref) setPreference(pPref);
    }
  }, [searchParams]);
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button
            ...
            style={{ 
              ...
              color: activeCustomerTab === 'reserve' ? '#FFFFFF' : 'var(--text-primary)',
              fontWeight: 600
            }}
          >
          ...
          {isCustomerAuth && (
            <button
              ...
              style={{ 
                ...
                color: activeCustomerTab === 'my-bookings' ? '#FFFFFF' : 'var(--text-primary)',
                fontWeight: 600
              }}
            >
      ...
      {activeCustomerTab === 'my-bookings' && isCustomerAuth ? (
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    color: historyTab === tab.key ? '#FFFFFF' : 'var(--text-secondary)',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.25rem' }}>
              {filteredBookings.map((b) => {
                ...
                if (b.status === 'Checked In') {
                  ...
                  badgeBg = 'var(--status-free-bg)';
                  badgeColor = 'var(--status-free)';
                } else if (b.status === 'Cancelled' || b.status === 'No Show') {
                  badgeBg = 'var(--status-occupied-bg)';
          ...
          <div className="panel-card">
            ...
            {combination && Array.isArray(combination.combination) && combination.combination.length > 0 && (
              <div style={{ marginTop: recommendations.length > 0 ? '1.5rem' : '0' }}>
                ...
                >
                  <span style={{ position: 'absolute', top: '-10px', left: '12px', fontSize: '0.7rem', padding: '0.1rem 0.5rem', backgroundColor: 'var(--accent-gold)', color: '#FFFFFF', borderRadius: 'var(--radius-sm)', fontWeight: 700, letterSpacing: '0.05em' }}>
