...
export default function ManagementPortal({ initialView = 'dashboard' }) {
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* ==========================================
         SIDEBAR NAVIGATION (STITCH UNIFIED SHELL)
         ========================================== */}
      <aside className="dashboard-sidebar">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
          <div className="logo-container" style={{ padding: '0.25rem 0.5rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Sparkles size={22} style={{ color: 'var(--accent-gold)' }} />
              <span style={{ fontSize: '1.35rem', fontWeight: 700, fontFamily: 'Geist, sans-serif', letterSpacing: '-0.03em' }}>DineSmart</span>
            </div>
            <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600, paddingLeft: '2px' }}>
              Management Suite
            </span>
          </div>
          ...
          <nav className="sidebar-nav">
            ...
            <button 
              className={`sidebar-btn ${currentView === 'waitlist' || currentView === 'seating' ? 'active' : ''}`}
              onClick={() => navigateToView('waitlist')}
            >
              <Users size={18} />
              <span>Waitlist & Seating</span>
            </button>
            <button 
        ...
        <div className="sidebar-footer">
          <div className="sidebar-profile">
            ...
            <div className="profile-info">
              <h5>{user?.name || 'Operations Manager'}</h5>
              <span style={{ fontSize: '0.7rem', color: 'var(--accent-gold)', fontWeight: 700, letterSpacing: '0.05em' }}>MANAGER</span>
            </div>
          </div>
          ...
          <button
            ...
            style={{ marginTop: '0.5rem', width: '100%', padding: '0.45rem', fontSize: '0.8rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem', borderRadius: 'var(--radius-sm)' }}
      ...
      <main className="dashboard-content">
        ...
        {/* Header Bar for Sub-Views (Stitch TopAppBar) */}
        {currentView !== 'dashboard' && currentView !== 'floor' && currentView !== 'waitlist' && currentView !== 'seating' && (
          <header className="dashboard-header stitch-app-bar">
            <div className="stitch-header-left">
              <h2 className="stitch-brand-title">The Gilded Bistro</h2>
              <div className="stitch-sub-meta">
                <span className="stitch-header-sub">
                  {currentView === 'bookings' && 'Reservations Registry • Guest Ledger'}
                  {currentView === 'history' && 'Reservation History • Master Audit Trail'}
                  {currentView === 'tables' && 'Table Inventory • Floor Layout Setup'}
                  {currentView === 'analytics' && 'Analytics & Business Intelligence • Service Telemetry'}
                </span>
                <span className="stitch-meta-divider">•</span>
                <span className="stitch-time-ticker">
                  {currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} • {currentTime.toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' })}
                </span>
                <span className="stitch-live-pill">
                  <span className="status-dot active" style={{ width: '6px', height: '6px' }}></span> Live Polling
                </span>
              </div>
            </div>
